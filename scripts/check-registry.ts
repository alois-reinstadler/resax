...
if (files.length !== 27) throw new Error(`Expected 27 JSON files (index + 26 items), found ${files.length}`);
for (const file of files) {
	...
	if (file === 'index.json') {
		if (!Array.isArray(value) || value.length !== 26) throw new Error('index.json: expected 26 item summaries');
		continue;
	}
