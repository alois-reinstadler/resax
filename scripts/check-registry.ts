...
if (files.length !== 20) throw new Error(`Expected 20 JSON files (index + 19 items), found ${files.length}`);
for (const file of files) {
	...
	if (file === 'index.json') {
		if (!Array.isArray(value) || value.length !== 19) throw new Error('index.json: expected nineteen item summaries');
		continue;
	}
