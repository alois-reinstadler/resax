...
if (files.length !== 26) throw new Error(`Expected 26 JSON files (index + 25 items), found ${files.length}`);
for (const file of files) {
	...
	if (file === 'index.json') {
		if (!Array.isArray(value) || value.length !== 25) throw new Error('index.json: expected 25 item summaries');
		continue;
	}
