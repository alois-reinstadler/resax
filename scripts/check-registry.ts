...
if (files.length !== 10) throw new Error(`Expected 10 JSON files (index + 9 items), found ${files.length}`);
for (const file of files) {
	...
	if (file === 'index.json') {
		if (!Array.isArray(value) || value.length !== 9) throw new Error('index.json: expected nine item summaries');
		continue;
	}
