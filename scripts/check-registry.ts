...
if (files.length !== 33) throw new Error(`Expected 33 JSON files (index + 32 items), found ${files.length}`);
for (const file of files) {
	...
	if (file === 'index.json') {
		if (!Array.isArray(value) || value.length !== 32) throw new Error('index.json: expected 32 item summaries');
		continue;
	}
