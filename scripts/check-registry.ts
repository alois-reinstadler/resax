...
if (files.length !== 31) throw new Error(`Expected 31 JSON files (index + 30 items), found ${files.length}`);
for (const file of files) {
	...
	if (file === 'index.json') {
		if (!Array.isArray(value) || value.length !== 30) throw new Error('index.json: expected 30 item summaries');
		continue;
	}
