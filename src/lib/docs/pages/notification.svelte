<script lang="ts">
	...
	let inlineTarget: HTMLDivElement;
	function showVariant(variant: NotifyVariant) {
		notify({ title: variant, text: 'Notification preview', variant, color: variant === 'snackbar' ? 'dark' : 'success', target: variant === 'inline' ? inlineTarget : undefined });
	}
	function updateDemo() {
...
<DemoSection title="Variants and colors" source={`notify({ text: 'Saved', variant: 'card', color: 'success' })`}><div class="button-demo-row" data-demo-section="variants">{#each variants as variant}<Button size="sm" color={variant === 'snackbar' ? 'dark' : 'primary'} onclick={() => showVariant(variant)}>{variant}</Button>{/each}</div><div class="inline-target" bind:this={inlineTarget}></div><p class="demo-note">The inline variant mounts into the target above, preserving the source component's in-flow layout. Other variants use their source edge positions.</p></DemoSection>
...
<DemoSection title="Goo surfaces and roundness" source={`notify({ title: 'Saved', text: 'Changes persisted', surface: 'glass', roundness: 20 })`}><div class="button-demo-row" data-demo-section="surfaces">{#each surfaces as surface}<Button size="sm" variant="flat" onclick={() => notify({ title: surface, text: 'Hover to expand the morphing body.', surface, roundness: surface==='glass'?22:16, position:'top-center', duration:5000 })}>{surface}</Button>{/each}</div></DemoSection>
...
<style>.snippet-message { display: inline-flex; align-items: center; gap: .5rem; }.inline-target{width:100%;max-width:480px;margin-top:14px}</style>
